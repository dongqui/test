...
const fnGetDeltaAppliedTracks = (props: FnGetDeltaAppliedTracks) => {
  ...
  const standardSubtractQuaternionTracks: THREE.QuaternionKeyframeTrack[] = [];
    ...
    const newValues = _.toArray(
      values.map(
        (value: number, idx: number) =>
          value - STANDARD_QUATERNIONS[boneName as StandardBoneName][idx % 4],
      ),
    );

    const newTrack = new THREE.QuaternionKeyframeTrack(name, newTimes, newValues);

    standardSubtractQuaternionTracks.push(newTrack);
  });

  const deltaAddedSourceQuaternionTracks: THREE.QuaternionKeyframeTrack[] = [];

  fnGetNormalizedQuaternionTracks({ quaternionTracks: standardSubtractQuaternionTracks }).forEach(
    (track: THREE.QuaternionKeyframeTrack) => {
      ...
      const newValues = _.toArray(
        values.map(
          (value: number, idx: number) =>
            value + targetQuaternions[boneName as StandardBoneName][idx % 4],
        ),
      );
