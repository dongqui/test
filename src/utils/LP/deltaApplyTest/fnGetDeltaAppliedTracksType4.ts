...
const fnGetDeltaAppliedTracks = (props: FnGetDeltaAppliedTracks) => {
    ...
    const newValues = _.toArray(
      values.map(
        (value: number, idx: number) =>
          value -
          STANDARD_EULERS[boneName as StandardBoneName][idx % 3] +
          targetEulers[boneName as StandardBoneName][idx % 3],
      ),
    );
  ...
  console.log('STANDARD_QUATERNIONS: ', STANDARD_QUATERNIONS);
  console.log('STANDARD_EULERS: ', STANDARD_EULERS);
  console.log('targetQuaternions: ', targetQuaternions);
  console.log('targetEulers: ', targetEulers);
  console.log('sourceRotationTracks: ', sourceRotationTracks);
  console.log('deltaAddedSourceRotationTracks: ', deltaAddedSourceRotationTracks);
